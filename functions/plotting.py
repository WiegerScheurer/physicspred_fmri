...
from functions.analysis import filter_condition
...
def plot_rt_performance_with_fit(n_trials, rt):
    """
    Plots n_trials on the x-axis and rt (reaction time) on the y-axis as dots.
    Skips NaN or None values in the rt list, fits a line to the valid data,
    and plots missing rt values as red dots on the x-axis.

    Parameters:
        n_trials (list or array): List of trial numbers (x-axis).
        rt (list or array): List of reaction times (y-axis).
    """
    # Convert to NumPy arrays for easier filtering
    n_trials = np.array(n_trials)
    rt = np.array(rt, dtype=np.float32)  # Convert to float to handle NaN

    # Filter out NaN or None values
    valid_indices = ~np.isnan(rt)  # Mask for valid (non-NaN) values
    n_trials_valid = n_trials[valid_indices]
    rt_valid = rt[valid_indices]

    # Fit a line to the valid data
    slope, intercept, _, _, _ = linregress(n_trials_valid, rt_valid)
    fitted_line = slope * n_trials_valid + intercept

    # Identify missing values
    missing_indices = np.isnan(rt)
    n_trials_missing = n_trials[missing_indices]

    # Plot the data
    plt.figure(figsize=(10, 6))
    plt.scatter(n_trials_valid, rt_valid, color='blue', label='Reaction Time (Valid)')
    plt.plot(n_trials_valid, fitted_line, color='green', linestyle='--', label='Fitted Line')
    plt.scatter(n_trials_missing, np.zeros_like(n_trials_missing), color='red', label='Missing RT', marker='x')
    plt.xlabel('Trial Number')
    plt.ylabel('Reaction Time (RT)')
    plt.title('Reaction Time Performance Across Trials')
