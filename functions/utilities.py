...
def create_balanced_trial_design(trial_n=None, change_ratio:list = [True, False], 
                                 ball_color_change_mean=0, ball_color_change_sd=0.05, 
                                 neg_bias_factor:float=1.5, neg_bias_shift:float=0.0, phant_bounce_directions:list=[],
                                 fmri_task:bool=False):
    ...
    # directions = ["left", "right"] * 8 # was this, but wrong, because then it's not fixed for direction

    direction_mapping = {
        "none_l_1": phant_bounce_directions[0], "none_l_2": phant_bounce_directions[0],
        "none_r_1": phant_bounce_directions[1], "none_r_2": phant_bounce_directions[1],
        "none_u_1": phant_bounce_directions[2], "none_u_2": phant_bounce_directions[2],
        "none_d_1": phant_bounce_directions[3], "none_d_2": phant_bounce_directions[3]
    }
    ...
    if trial_n is not None:
        ...
        # Set the ball_luminance value to None if ball_change is False
        for trial in all_trials:
            if not trial['ball_change']:
                trial['ball_luminance'] = None
...
def build_design_matrix(n_trials:int, change_ratio:list=[True, False], 
                        ...
                        fmri_task:bool=False, prompt_every:int=None):
    """
    ...
    if trials_per_fullmx is None:
        test_dm = create_balanced_trial_design(trial_n=None, 
                                               ...
                                               phant_bounce_directions=directions,
                                               fmri_task=fmri_task)
    ...
    # Call on itself to create a separately balanced subset of task prompt trials
    # Including a new column 'task_prompt' to indicate task prompt trials
    if fmri_task is True and prompt_every is not None:
        fmri_dmx = design_matrix.copy()
        task_dmx = build_design_matrix(
            n_trials=n_trials // prompt_every, 
            ...
            neg_bias_shift=neg_bias_shift,
            fmri_task = True,
            prompt_every = None, # Set to None so not to initiate an infinite loop
        )
        # Add a 'prompt' column to the design matrix
        fmri_dmx["task_prompt"] = False
        task_dmx["task_prompt"] = True

        # Concatenate the dmxs
        concat_dmx = pd.concat([fmri_dmx, task_dmx], ignore_index=True)
        # Shuffle the rows and reset indices
        design_matrix = concat_dmx.sample(frac=1).reset_index(drop=True)
...
def check_balance(df):
    ...
    for option in sorted(df[df['trial_type'] == 'empty']['trial_option'].unique()):
        ...
        print(f"  Ball Change: {subset['ball_change'].value_counts().to_dict()}")
        print(f"  Ball luminance: {subset['ball_luminance'].value_counts().to_dict()}")
        print(f"  Bounce direction: {subset['phant_bounce_direction'].value_counts().to_dict()}")
...
def save_performance_data(subject_id, task_name, data, design_matrix:bool=False, 
                          intermediate:bool=False, base_dir="/Users/wiegerscheurer/repos/physicspred/database", session="1"):
    """
...
def get_pos_and_dirs(ball_speed, square_size, ball_radius):
    ...
    return start_positions, directions
