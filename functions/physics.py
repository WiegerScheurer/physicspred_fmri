...
def collide(start_direction: str, line_angle: int, ball_speed: float):
    ...
    """
    direction_angles = {"up": 270, "down": 90, "left": 0, "right": 180}
...
def calculate_decay_factor(start_speed, elapsed_time, total_time, constant=(0.01 / 6)):
    ...
    # constant = 1
    decay_rate = (
        start_speed * constant
    )  # Adjust this value to control the rate of decay
    return np.exp(-decay_rate * (elapsed_time / total_time))


def will_cross_fixation(ball_pos, velocity, skip_factor):
    # Calculate the next position
    next_pos = ball_pos + np.array(
        [velocity[0] * skip_factor, velocity[1] * skip_factor]
    )
    ...
    if np.sign(ball_pos[0]) != np.sign(next_pos[0]) or np.sign(ball_pos[1]) != np.sign(
        next_pos[1]
    ):
        return True
    return False
...
def _vec_to_dir(vector: tuple) -> str:
    ...
    """
    vectors = {(1, 0): "up", (0, 1): "right", (-1, 0): "down", (0, -1): "left"}
...
def _dir_to_loc(direction: str) -> tuple:
    ...
    """
    locations = {"up": (0, 1), "right": (1, 2), "down": (2, 1), "left": (1, 0)}
...
def _dir_to_velocity(direction: str | tuple, speed: float) -> tuple:
    ...
    """
    direction_vector = (
        _dir_to_vec(direction) if isinstance(direction, str) else direction
    )
...
def _rotate_90(start_direction, left_or_right):
    ...
    towards = {"left": rotation_matrix_270, "right": rotation_matrix_90}
...
def _flip_dir(direction: str | tuple) -> str | tuple:
    ...
    if isinstance(direction, str):
        flipped_dir = _vec_to_dir(
            tuple(dir_axis * -1 for dir_axis in _dir_to_vec(direction))
        )
...
def _bounce_ball(start_direction: str, interactor: str, str_or_tuple_out: str = "str"):
    ...
    if interactor[:3] == "135":
        ...
        end_loc = _rotate_90(
            start_direction=start_direction, left_or_right=relative_direction
        )
        ...
        end_loc = _dir_to_vec(
            start_direction
        )  # When no interactor, ball ends up in the same direction
...
def plot_positions(start_pos, end_pos, pred_to_input, interactor):
    ...
    start_coords = positions[start_pos]
    end_coords = positions[end_pos]
    # Add trajectory with a 90-degree angle (via the center)
    center_coords = (0, 0)
    ax.plot(
        [start_coords[0], center_coords[0]],
        [start_coords[1], center_coords[1]],
        "k:",
        alpha=0.7,
    )
    ax.plot(
        [center_coords[0], end_coords[0]],
        [center_coords[1], end_coords[1]],
        "k:",
    ...
    # Plot end position
    ax.plot(
        end_coords[0], end_coords[1], "ro", markersize=13, label="End Position", alpha=1
    )
    # Plot predicted positions
    for pos, value in pred_to_input.items():
        if value[0] == 1:
    ...
    # Add bounce stripe based on interactor value
    if interactor[:3] == "135":
        ax.plot([-0.2, 0.2], [-0.2, 0.2], "k-", label="135° interactor")
    elif interactor[:2] == "45":
        ax.plot([-0.2, 0.2], [0.2, -0.2], "k-", label="45° interactor")
...
def predict_ball_path(
    hypothesis: str, interactor: str, start_pos: str, end_pos: str, plot: bool = False
):
    """
    Predict the path of a ball based on the given parameters.
    ...
    Returns:
        dict: A dictionary representing the path of the ball.
    """
    pred_to_input = {"up": [0], "right": [0], "down": [0], "left": [0]}
    ...
    if hypothesis == "abs":
        ...
        # NOTE: flip_dir is used to get the ball direction based on the start location
        predicted_dir = _bounce_ball(
            start_direction=_flip_dir(start_pos), interactor=interactor
        )
        predicted_endloc = _flip_dir(predicted_dir)  # Flip direction to get endpoint
        pred_to_input[predicted_endloc] = [1]
        # pred_to_input[predicted_dir] = [1]
...
def correct_simpreds(raw_datfram:pd.DataFrame, plot:bool=False, verbose:bool=False) -> pd.DataFrame:
    datfram = raw_datfram.copy()
    
    # Loop over the two different interactors (45 and 135 degrees)
    for interactor in ["45", "135"]:
        
        subset_ices = datfram.index[datfram["interactor"].str.startswith(interactor) & (datfram["bounce"] == True)]
        random_trial = subset_ices[random.randint(0, len(subset_ices) - 1)]


        for index in [random_trial]:#subset_ices:
            print(f"Trial: {index}") if verbose else None
            datfram.loc[index, "end_pos"] = _flip_dir(datfram.loc[index, "end_pos"])
        print(f"Random trial: {random_trial}")

        if plot:
            for df_no, this_df in enumerate([raw_datfram, datfram]):
                to_be = "was" if df_no == 0 else "became"
                print(f"Trial {random_trial} end pos {to_be}: {datfram.loc[index, 'end_pos']}")
                _ = predict_ball_path(
                    hypothesis="sim",
                    interactor=interactor,
                    start_pos=this_df.loc[random_trial, "start_pos"],
                    end_pos=this_df.loc[random_trial, "end_pos"],
                    plot=True,)
