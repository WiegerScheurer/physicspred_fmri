# # This I copied from the core, but shouldn't do that, because not necessary.
...
#                        create_cross_fixation, 
#                        create_occluder, 
#                        create_screen_borders,
#                        create_ball)
...
#         #     win=self.win,
#         #     radius=self.settings["ball"]["radius"],
#         #     )


#         # So these if statements are to make sure that the
#         # session works when called from within different repositories
#         # However, I feel like it's not convenient, and there's also
#         # session files in the cloned repositories (I believe), so perhaps
#         # that's something to figure out: how to make a script use a "local"
#         # session.py file, rather than the one from the exptools2 package
#         # in the site-packages folder of the conda env activated.
#         if "fixation" in self.settings:
#             self.cross_fix = create_cross_fixation(
#                 self.win,
#                 **self.settings["fixation"]
#             )
#         else:
#             self.cross_fix = None  # Or some default behavior

#         if "occluder" in self.settings:
#             self.occluder = create_occluder(
#                 win=self.win,
#                 **self.settings["occluder"]
#             )
...
#         if "display" in self.settings and "win_dims" in self.settings["display"] and "square_size" in self.settings["display"]:
#             self.screen_borders = create_screen_borders(
#                 win=self.win,
#                 dims=self.settings["display"]["win_dims"],
#                 square_size=self.settings["display"]["square_size"]
...
#         if "ball" in self.settings and "radius" in self.settings["ball"]:
#             self.ball = create_ball(
#                 win=self.win,
#                 radius=self.settings["ball"]["radius"]
#             )
