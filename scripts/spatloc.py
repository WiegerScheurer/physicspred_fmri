...
sys.path.append(
    "/Users/wieger.scheurer/exp_venv/lib/python3.10/site-packages/exptools2"
)
sys.path.append(
    "/Users/wieger.scheurer/exp_venv/lib/python3.10/site-packages/exptools2/experiments/physicspred/"
)
...
class BallTrial(Trial):
    ...
    def __init__(self, session, trial_nr, phase_durations, trial_params, **kwargs):
        ...
        self.changed_ball_color = oklab_to_rgb(
            [(self.ball_start_color + self.ball_color_change), 0, 0], psychopy_rgb=True
        )
        ...
        self.edge = _flip_dir(
            next(option for option in edge_options if option.startswith(edge_letter))
        )
    ...
    def prepare_trial(self):
        ...
        self.session.ball.color = np.clip(
            oklab_to_rgb([self.ball_start_color, 0, 0], psychopy_rgb=True), -1, 1
        )
    ...
    def draw(self):
        ...
        if self.phase == 0:  # Only occluder phase
            ...
            while self.session.timer.getTime() < 0:
                ...
                spawn_range, n_stutters, n_movs, stut_intervals, mov_variance = (
                    self.spatloc_stimbuild()
                    if spawn_range is None
                    else (spawn_range, n_stutters, n_movs, stut_intervals, mov_variance)
                )

                (
                    print(
                        f"Spawn range: {spawn_range}, n_stutters: {n_stutters}, n_movs: {n_movs}"
                    )
                    if not started_rolling
                    else None
                )
                # Process ball movement
                self.spatloc_ballmov(
                    location=self.block["location"],
            ...
            self.phase == 2
        ):  # Inter trial interval, perhaps this should be phase 0, but doesn't matter
    ...
    def spatloc_stimbuild(self):
        ...
        n_stutters = int(
            self.phase_durations[1]
            // (
                self.session.config.timing.stutter_interval
                + self.session.config.timing.stutter_dur
            )
        )
        n_movs = int(
            self.phase_durations[1] // (self.session.config.timing.movement_dur)
        )

        # Calculate the inner and outer bounds for spawning, based on the occluder and ball radius
        inner_bound = self.session.config.occluder.radius // 2 + (
            self.session.config.ball.radius // 2
        )
        outer_bound = self.session.config.display.square_size // 2 - (
            self.session.config.ball.radius // 2
        )
            ...
            for i in range(n_numbers):
                ...
                for attempt in range(max_attempts_per_sample):
                    if jitter_within_segment:
                        ...
                        ) / 2  # midden van segment
                    candidate = int(candidate)

                    # Check globale afstand
                    global_ok = all(
                        abs(candidate - num) >= min_distance for num in numbers
                    )
                    # Check lokale afstand
                    if numbers:
                        local_ok = (
                            abs(candidate - numbers[-1]) >= subsequent_min_distance
                        )
                    else:
        ...
        stut_per_loc = 3
        spawn_range_locs = generate_evenly_spread_separated_numbers(
            inner_bound,
            outer_bound,
            (n_stutters // stut_per_loc) + stut_per_loc,
            min_distance,
            subsequent_min_distance,
        )  # + margin of stutperloc
    ...
    def spatloc_ballmov(
        ...
        loc_map = {"left": (-1, 0), "right": (1, 0), "top": (0, 1), "bottom": (0, -1)}
        ...
        movement_amplitude = (
            0.51 * self.session.config.display.square_size
            + self.session.config.ball.radius
        )
        ...
        if movement == "stutter":
            ...
            for i in range(n_events):
                if i < len(stut_intervals):
                    ...
                    interval = (
                        self.session.config.stutter_interval
                    )  # Default if not enough provided

                # Each event consists of a visible period followed by an interval
                stutter_timings.append(
                    {"start": start_time, "end": start_time + stutter_dur, "index": i}
                )
            ...
            if current_stutter is not None:
                ...
                self.session.ball.pos = tuple(
                    np.array(loc_map[location]) * spawn_range[event_idx]
                )
                self.session.ball.opacity = 1  # Make ball visible
                ...
                if ball_task and event_idx in target_indices:
                    ...
                    try:
                        ...
                        if hasattr(self.session.ball, "fillColor"):
                            ...
                            if self.session.ball.colorSpace == "rgb":
                                ...
                                self.session.ball.fillColor = [
                                    min(
                                        ...
                                        max(
                                            -1.0,
                                            self.session.ball.fillColor[i]
                                            + target_luminance_change,
                                        ),
                                    )
                            ...
                            if (
                                hasattr(self.session.ball, "colorSpace")
                                and self.session.ball.colorSpace == "rgb"
                            ):
            ...
            actual_durations = [
                movement_duration + var for var in mov_variance[:n_events]
            ]
            ...
            for i in range(1, n_events):
                movement_start_times.append(
                    movement_start_times[i - 1] + actual_durations[i - 1]
                )
            ...
            for i in range(n_events):
                if i == n_events - 1:  # Last movement
                    if (
                        movement_start_times[i]
                        <= elapsed
                        < (movement_start_times[i] + actual_durations[i])
                    ):
                        current_movement = i
                        break
                else:
            ...
            movement_progress = (
                elapsed - movement_start_times[current_movement]
            ) / actual_durations[current_movement]
    ...
    def process_ball_movement(self):
        ...
        self.session.ball.pos += tuple(
            [self.velocity[0] * 1, self.velocity[1] * 1]
        )  # Using skip_factor=1
        ...
        if (
            ...
            and self.left_occluder
            and not self.hue_changed
        ):
...
# class SpatLocSession(Session):
class SpatLocSession(PylinkEyetrackerSession):
    ...
    def setup_visual_elements(self):
        ...
        border_width = (
            config.display.win_dims[0] - square_size
        ) / 2  # config.display.border_width
        ...
        bounce_dist = get_bounce_dist(
            config.ball.radius + (config.interactor.width / 2 * 1.8)
        )  # 1.8 factor is due to the that now we use an image
    ...
    def create_design(self):
        ...
        ball_speeds = bellshape_sample(
            float(config.ball.avg_speed),
            float(config.ball.natural_speed_variance),
            n_trials,
        )
        ball_start_colors = bellshape_sample(
            float(config.ball.start_color_mean),
            float(config.ball.start_color_sd),
            n_trials,
        )

        # Generate inter-trial intervals, but now already done in the trial setup func, log there
        itis = truncated_exponential_decay(
            config.timing.min_iti, config.timing.max_iti, n_trials
        )
    ...
    def create_trials(self, n_trials=None):
        ...
        itis = truncated_exponential_decay(
            min_iti=self.config.timing.min_iti,
        ...
        for trial_nr in range(n_trials):
            ...
            self.trials[
                trial_nr
            ].prepare_trial()  # Prepare each trial # NOT SURE IF RIGHT PLACE
    ...
    def run(self, run_no: int = None):
        ...
        for trial_idx, trial in enumerate(self.trials):
            ...
            print(
                f"Trial: {trial.trial_nr}, Location: {trial.block['location']}, Movement: {trial.block['movement']}"
            )
...
#     session = SpatLocSession(output_str="sub-test", config_file=settings, settings_file=settings)
...
if __name__ == "__main__":
    ...
    info = {"Subject": "sub-01", "Run": 1, "Eyetracker": True}
    dlg = gui.DlgFromDict(info, title="Spatial Localiser Initialisation")
    if not dlg.OK:
        print("The experiment was quit.")
        exit()

    config_path = os.path.join(
        os.path.dirname(__file__), os.pardir, "behav_settings.yml"
    )

    # Create and run the session
    session = SpatLocSession(
    ...
    session.create_trials(
        n_trials=session.config.experiment.n_trials_spatloc
